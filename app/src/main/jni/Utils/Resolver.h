...
public:
    static bool initialize() {
        ...
        if (!inst) {
            LOGE(OBFUSCATE("Couldn't create instance"));
            return false;
        }
        if (!inst->il2cpp_resolve_icall) {
            SigUtils::LibraryInfo info = SigUtils::findLibraryInfo(OBFUSCATE("il2cpp.so"));
            if (info.startAddr != 0 && info.endAddr != 0) {
                ...
                *reinterpret_cast<void **>(&inst->il2cpp_resolve_icall) = SigUtils::tryFind(
                ...
                LOGI(OBFUSCATE("il2cpp_resolve_icall address: %p"), inst->il2cpp_resolve_icall);
                return inst->il2cpp_resolve_icall != NULL;
            } else {
                LOGE(OBFUSCATE("Couldn't find il2cpp map!"));
                return false;
            }
        }
		return false;
    ...
    static void *ResolveCall(std::string funcName) {
        ...
        if (!inst->il2cpp_resolve_icall) {
            LOGE(OBFUSCATE("il2cpp_resolve_icall is null!"));
            return NULL;
        }
        ...
        LOGI(OBFUSCATE("%s address: %p"), funcName.c_str(), ret);
        return ret;
    }
};
